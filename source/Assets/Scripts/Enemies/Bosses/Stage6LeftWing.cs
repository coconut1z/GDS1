...
public class Stage6LeftWing : MonoBehaviour {
	...
	void Update () {
		...
		if(!inPosition && GetComponent<ReturnToOriginalPosition>().timer >= 1){
			inPosition = true;
			transform.GetChild (0).GetComponent<Stage6LaserGun> ().ChangePhase (1);
			transform.GetChild (1).GetComponent<Stage6LaserGun> ().ChangePhase (1);
		}
		if(finalFade){
	...
	public void ReturnToDefault(){
		transform.GetChild (0).GetComponent<Stage6LaserGun> ().ChangePhase (0);
		transform.GetChild (1).GetComponent<Stage6LaserGun> ().ChangePhase (0);
	}
