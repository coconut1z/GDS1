...
public class CannonMkIII : WeaponModule {
	...
	public override void Shoot(){
		if(shootTime > shootDelay){
			...
			Instantiate (projectiles [1], shootPos [0].position, 
				Quaternion.Euler (0, 0, transform.eulerAngles.z + 7f));
			Instantiate (projectiles [0], shootPos [1].position, 
				Quaternion.Euler (0, 0, transform.eulerAngles.z));
            Instantiate(projectiles[1], shootPos[2].position,
                Quaternion.Euler(0, 0, transform.eulerAngles.z - 7f));
            shootTime = 0;
