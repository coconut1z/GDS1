...
public class CompanionDroneII : MonoBehaviour {
    ...
    void Start() {
        damage = 3.0f; //was 15dmg //was 2.3, buffed due to stage 2 wep -J
        speed = 5.0f; //was 4.0f
        rotateSpeed = 500.0f;
        ...
        fg = GetComponentInChildren<CompDroneIIShotty>();
        randomTimer = 0;
        targetOffset = false;
        StartHunt();
    }
    ...
    private void FixedUpdate() {
        ...
        //using normal movement, just targeting the orbiter with them now.
    ...
    private void UpdateMousePos() {
        /*mousePos = new Vector2(Camera.main.ScreenToWorldPoint(Input.mousePosition).x,
            ...
            //removing mouse, adding orbiter :D
        mousePos = orbit.transform.position;
    }
    ...
    private void EmergencyTP() {
        ...
        if(vp.x > 9 || vp.x < -9 || vp.y > 8 || vp.y < -8) {
            ...
            */
            //Removed from Companion drone due to lack of need.
    ...
    void OnDrawGizmos() {
        Gizmos.DrawSphere(orbit.position, 0.1f);
        Gizmos.DrawLine(orbit.position, transform.position);
    }
}
